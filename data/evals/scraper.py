...
# The <tr> in the last <table>
				critique["class_size"] = int(result[i].contents[5].contents[0].lstrip("Class Size: "))
				critique["total_replies"] = int(result[i].contents[3].contents[0].lstrip("Total replies: "))
				critique["scores"] = scores

				course_dict["critiques"].append(critique)
			elif len(result[i]) == 20:
# The interesting rows of result[i] just happen to have 20 elements in .contents
				q = int(result[i].contents[0].contents[0].contents[0])
				scores[q] = {}
				scores[q]["num_replies"] = int(result[i].contents[2].contents[0])
